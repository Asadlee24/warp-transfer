...
// A wider spread of STUN servers + a pre-warmed candidate pool cuts down
// noticeably on the time WebRTC spends gathering ICE candidates before
// the connection can actually open.
const RTC_CONFIG = {
  iceServers: [
  ...
  const ensurePeer = useCallback((id, onUnrecoverable) => {
    return new Promise((resolve, reject) => {
      import("peerjs").then(({ default: Peer }) => {
        const peer = id
          ? new Peer(id, { config: RTC_CONFIG })
          : new Peer({ config: RTC_CONFIG });
