...
export function useTransfer() {
  ...
  // Registers a PeerJS peer and WAITS for it to open on the signaling broker
  // before resolving. This ensures the sender is fully registered before we
  // reveal the code to the user — so receivers can always find it immediately.
  ...
  const startSending = useCallback(
    async (fileOrFiles) => {
      ...
      // Use a plain 6-char code as the peer ID (no prefix) — same as original working code
      ...
      try {
        // Wait for peer to be fully registered on broker BEFORE showing code
        const { peer } = await ensurePeer(shortCode, (err) => fail(err.message));
        if (myRunId !== runIdRef.current) return;

        // Only set the code AFTER broker confirms registration
        setCode(shortCode);
  ...
  const startReceiving = useCallback(
    async (targetCode) => {
      cleanup();
      const myRunId = runIdRef.current;
      // Strip any URL, extract only the raw 6-char code
      ...
      try {
        // Get a random peer ID for the receiver (not the sender's code)
        const { peer } = await ensurePeer(null, (err) => fail(err.message));
        if (myRunId !== runIdRef.current) return;

        setStatusText(`Connecting to sender [${cleanCode}]...`);

        // Connect directly to sender's code — same plain code the sender registered with
        const conn = peer.connect(cleanCode, { reliable: true });
        connRef.current = conn;

        connectTimeoutRef.current = setTimeout(() => {
          fail("Connection timed out. Make sure the sender's tab is open and the code is correct.");
        }, CONNECT_TIMEOUT_MS);
  ...
  return { status, statusText, role, code, progress, fileMeta, errorMsg, receivedFiles, reconnecting, speedBps, etaSeconds, startSending, startReceiving, reset };
}
