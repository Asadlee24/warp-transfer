...
function waitForDrainConns(conns, myRunId, runIdRef) {
  const promises = conns.map((conn) => {
...
export function useTransfer() {
  ...
  const connectionsRef = useRef([]); // Multi-receiver connections list for sender
  const receiverConnRef = useRef(null); // Single connection for receiver
  ...
  const cleanup = useCallback(() => {
    ...
    try { receiverConnRef.current?.close?.(); } catch (e) {}
    try { peerRef.current?.destroy?.(); } catch (e) {}
    peerRef.current = null;
    connectionsRef.current = [];
    receiverConnRef.current = null;
  ...
  // Multicast send loop for sender
  const sendFilesToConnections = useCallback(
    async (connsToStream, files, myRunId) => {
      if (!files || !files.length || !connsToStream.length) return;
      isTransferringRef.current = true;
      resetSpeedTracking();
      const totalBytes = files.reduce((sum, f) => sum + f.size, 0);
      let sentBeforeCurrent = 0;

      connsToStream.forEach((conn) => {
        const dc = conn.dataChannel;
      ...
      try {
        for (let i = 0; i < files.length; i++) {
          ...
          const count = connsToStream.length;
          setStatusText(`Streaming to ${count} receiver${count > 1 ? "s" : ""}: ${file.name}`);
          ...
          // Multicast metadata
          connsToStream.forEach((conn) => {
            if (conn.open) conn.send(metaMsg);
          });

          let offset = 0;
          while (offset < file.size) {
            if (myRunId !== runIdRef.current) return;
            await waitForDrainConns(connsToStream, myRunId, runIdRef);
            if (myRunId !== runIdRef.current) return;

            const buf = await file.slice(offset, offset + CHUNK_SIZE).arrayBuffer();

            // Broadcast chunk to all active receivers
            connsToStream.forEach((conn) => {
              if (conn.open) {
          ...
          connsToStream.forEach((conn) => {
            if (conn.open) conn.send({ type: "file-done" });
          });
          sentBeforeCurrent += file.size;
        }

        connsToStream.forEach((conn) => {
          if (conn.open) conn.send({ type: "all-done" });
        });

        setStatus("done");
        const count = connsToStream.length;
        setStatusText(`Delivered to ${count} receiver${count > 1 ? "s" : ""}!`);
        setProgress(100);
        setEtaSeconds(0);
        sounds.complete();
        clearHeartbeat();
  ...
  const startSending = useCallback(
    async (fileOrFiles) => {
      ...
      try {
        ...
        setCode(shortCode);
        setStatusText("Waiting for receivers (Multi-Stream Ready)...");

        // Handle multiple incoming receivers!
        peer.on("connection", (conn) => {
          ...
          const onOpen = () => {
            ...
            // Add new receiver to connections array if not present
            if (!connectionsRef.current.some((c) => c.peer === conn.peer)) {
              connectionsRef.current.push(conn);
            }

            const currentCount = connectionsRef.current.length;
            setReceiverCount(currentCount);
            dataChannelActiveRef.current = true;
            sounds.connect();

            setStatus("connected");
            setStatusText(
              `${currentCount} receiver${currentCount > 1 ? "s" : ""} connected! Streaming...`
            );

            // Trigger stream if not already actively transferring
            if (!isTransferringRef.current) {
              sendFilesToConnections(connectionsRef.current, files, myRunId);
            } else {
              // If already transferring, stream current file list to newly joined receiver as well
              sendFilesToConnections([conn], files, myRunId);
            }
          };
          ...
          conn.on("close", () => {
            ...
            const remainingCount = connectionsRef.current.length;
            setReceiverCount(remainingCount);

            if (remainingCount === 0 && !isTransferringRef.current && status !== "done") {
              setStatus("waiting");
    ...
    [ensurePeer, cleanup, sendFilesToConnections]
  );
